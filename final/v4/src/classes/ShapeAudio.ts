 ...
 * Higher Y (bottom) = lower frequency (bass), Lower Y (top) = higher frequency (treble)
 */
export function getFrequencyForY(y: number): {
  ...
  // Reverse the index: top of screen = high freq, bottom = low freq
  const reversedIndex = numLines - 1 - clampedIndex;

  // Line Y position (1-indexed from top)
  const lineY = (clampedIndex + 1) * lineSpacing;

  return {
    freq: notes[reversedIndex]?.freq ?? 220,
 ...
 * Returns lines with high frequencies at top, low frequencies at bottom.
 */
export function getFrequencyLinePositions(): {
  ...
  // Reverse mapping: high freq notes at top (low Y), low freq notes at bottom (high Y)
  return notes.map((note, i) => ({
    y: (numLines - i) * lineSpacing,
    freq: note.freq,
    name: note.name,
  }));
}
...
export class ShapeAudio {
  ...
  // User-controlled gain (0-1), separate from mixer's automatic gain
  private userGain: number = 1.0;
  private mixerGain: number = 1.0; // Store the mixer's recommended gain

  // Wave amplitude scale (affects modulation depth, not normalized away)
  private waveAmplitudeScale: number = 1.0;
  private modulatorScaleGain: GainNode | null = null; // For AM mode modulation depth
  ...
  private initWaveformAudio(): void {
    ...
    // 4. Modulator scale gain (controls AM depth, affected by waveAmplitudeScale)
    this.modulatorScaleGain = ctx.createGain();
    this.modulatorScaleGain.gain.value = this.waveAmplitudeScale;

    carrier.connect(vca);
    modulator.connect(this.modulatorScaleGain);
    this.modulatorScaleGain.connect(vca.gain); // Scaled modulator controls the volume
  ...
  private initFMAudio(): void {
    ...
    // Also scaled by waveAmplitudeScale for wobble control
    this.fmModGain = ctx.createGain();
    this.fmModGain.gain.value =
      this.currentFrequency * 0.5 * this.waveAmplitudeScale;
  ...
  updateFMFrequencyFromY(worldY: number): void {
    ...
    // Apply waveAmplitudeScale for wobble control
    if (this.fmModGain) {
      this.fmModGain.gain.setTargetAtTime(
        freq * 0.5 * this.waveAmplitudeScale,
        ctx.currentTime,
        0.02
      );
    }
  ...
  /**
   * Updates the loop duration based on a new effective perimeter (original perimeter * scale).
   * Recalculates loopBars and updates the modulator playback rate.
   */
  updateEffectivePerimeter(effectivePerimeter: number): void {
    // Recalculate loop bars based on the new effective perimeter
    const perimeterRatio = effectivePerimeter / this.referencePerimeter;
    const newLoopBars = snapToIntegerRatio(perimeterRatio);

    if (newLoopBars === this.loopBars) return; // No change needed

    this.loopBars = newLoopBars;
    console.log(
      'Updated loopBars:',
      this.loopBars,
      'from effective perimeter:',
      effectivePerimeter
    );

    // Update modulator playback rate to match new loop duration
    const barDuration = Metronome.getBarDuration();
  ...
  /**
   * Gets the current loop bars value.
   */
  getLoopBars(): number {
  ...
  /**
   * Gets the original (unscaled) perimeter.
   */
  getPerimeter(): number {
  ...
  /**
   * Sets the user-controlled gain (0-1).
   * This is multiplied with the mixer's automatic gain.
   */
  setUserGain(gain: number): void {
    this.userGain = Math.max(0, Math.min(5, gain));
    if (this.gainNode) {
      const ctx = this.p.getAudioContext() as unknown as AudioContext;
      this.gainNode.gain.setTargetAtTime(
        this.mixerGain * this.userGain,
        ctx.currentTime,
        0.02
      );
    }
  ...
  /**
   * Gets the current user-controlled gain (0-1).
   */
  getUserGain(): number {
  ...
  /**
   * Updates the wave data and reinitializes audio.
   * Used when wave amplitude scale changes.
   */
  updateWave(newWave: WaveSamplePoint[], amplitudeScale?: number): void {
    this.wave = newWave;
    if (amplitudeScale !== undefined) {
      this.waveAmplitudeScale = amplitudeScale;
    }
    // Reinitialize audio with new wave (preserves userGain)
    const savedUserGain = this.userGain;

    // Unregister from mixer before disposing (initAudio will re-register)
    AudioMixer.unregisterSource();
    this.disposeAudioOnly();
    this.initAudio();

    this.userGain = savedUserGain;
    // Apply saved gain
    if (this.gainNode) {
      this.gainNode.gain.value = this.mixerGain * this.userGain;
    }
  ...
  /**
   * Sets the wave amplitude scale (wobble intensity) in real-time.
   * This affects the modulation depth without reinitializing audio.
   */
  setWaveAmplitudeScale(scale: number): void {
    this.waveAmplitudeScale = scale;
    const ctx = this.p.getAudioContext() as unknown as AudioContext;

    // Update FM modulation depth
    if (settings.SYNTHESIS_MODE === 'fm' && this.fmModGain) {
      this.fmModGain.gain.setTargetAtTime(
        this.currentFrequency * 0.5 * this.waveAmplitudeScale,
        ctx.currentTime,
        0.02
    ...
    // Update AM modulation depth
    if (settings.SYNTHESIS_MODE === 'waveform' && this.modulatorScaleGain) {
      this.modulatorScaleGain.gain.setTargetAtTime(
        this.waveAmplitudeScale,
        ctx.currentTime,
        0.02
      );
    }
  ...
  /**
   * Disposes audio without unregistering from mixer (for wave updates).
   */
  private disposeAudioOnly(): void {
    ...
    if (this.modulatorScaleGain) {
      this.modulatorScaleGain.disconnect();
      this.modulatorScaleGain = null;
    }
